...
// Fetch subject
$stmt_fetch = mysqli_prepare($koneksi, "SELECT * FROM mata_kuliah WHERE id = ?");
...
if (isset($_POST['update'])) {
    ...
    if (empty($kode_mk) || empty($nama_mk) || empty($sks)) {
        ...
        $error = 'Bobot SKS tidak valid!';
    } else {
        // Check for duplicate Kode MK on other rows
        $stmt_check = mysqli_prepare($koneksi, "SELECT id FROM mata_kuliah WHERE kode_mk = ? AND id != ?");
        mysqli_stmt_bind_param($stmt_check, "si", $kode_mk, $id);
        mysqli_stmt_execute($stmt_check);
        mysqli_stmt_store_result($stmt_check);
        
        if (mysqli_stmt_num_rows($stmt_check) > 0) {
            $error = "Kode MK $kode_mk sudah terdaftar oleh mata kuliah lain!";
            mysqli_stmt_close($stmt_check);
        } else {
            mysqli_stmt_close($stmt_check);
            
            // Update
            $stmt_update = mysqli_prepare($koneksi, "UPDATE mata_kuliah SET kode_mk = ?, nama_mk = ?, sks = ? WHERE id = ?");
            mysqli_stmt_bind_param($stmt_update, "ssii", $kode_mk, $nama_mk, $sks, $id);
...
<div class="main-content">
    ...
    <h3 class="page-title">Edit Data Mata Kuliah</h3>
        ...
        <form method="post" action="">
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" id="kode_mk" name="kode_mk" value="<?= htmlspecialchars($d['kode_mk']) ?>" required>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="sks" name="sks" required>
                        ...
                        <option value="1" <?= $d['sks'] == 1 ? 'selected' : '' ?>>1 SKS</option>
                        <option value="2" <?= $d['sks'] == 2 ? 'selected' : '' ?>>2 SKS</option>
                        <option value="3" <?= $d['sks'] == 3 ? 'selected' : '' ?>>3 SKS</option>
                        <option value="4" <?= $d['sks'] == 4 ? 'selected' : '' ?>>4 SKS</option>
                        <option value="6" <?= $d['sks'] == 6 ? 'selected' : '' ?>>6 SKS</option>
                    </select>
                </div>
                ...
                <div class="col-12">
                    ...
                    <input type="text" class="form-control" id="nama_mk" name="nama_mk" value="<?= htmlspecialchars($d['nama_mk']) ?>" required>
                </div>
            </div>
